...
export class RazorDocumentSynchronizer {
    ...
    public register() {
        return vscode.workspace.onDidChangeTextDocument(event => this.textDocumentChanged(event));
    }
    ...
    Currently at ${projectedDocument.hostDocumentSyncVersion}, synchronizing to version '${toHostDocumentVersion}'.
    ...
    Current projected document version: '${projectedDocument.projectedDocumentSyncVersion}'`);
        ...
        if (hostDocument.version !== toHostDocumentVersion) {
            if (this.logger.verboseEnabled) {
                this.logger.logVerbose(
        ...
        try {
            if (projectedDocument.hostDocumentSyncVersion !== toHostDocumentVersion) {
                if (this.logger.verboseEnabled) {
                    this.logger.logVerbose(
    ...
    private removeSynchronization(context: SynchronizationContext) {
        ...
        for (let i = 0; i < synchronizations.length; i++) {
            if (synchronizations[i] === context) {
                const newSynchronizations = synchronizations.splice(i, 1);
                this.synchronizations[documentKey] = newSynchronizations;
                break;
            }
        }
    }
